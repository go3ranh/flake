{ inputs, pkgs, lib, self, flake-utils }:
builtins.foldl'
  (result: name:
    let
  ...
  settings = pkgs.stdenv.mkDerivation
    ...
    (system:
      let syspkgs = inputs.nixpkgs.legacyPackages.${system}; in
      {
        buildInputs = [ pkgs.fzf ];
        name = "settings";
        description = "goeranh settings / dotfiles";
        bashrc = pkgs.writeText ".bashrc" ''
          source "${syspkgs.fzf.outPath}/share/fzf/key-bindings.bash"
          source "${syspkgs.fzf.outPath}/share/fzf/completion.bash"
          function pkgsearch (){
          nix-env -qa | fzf
          }
